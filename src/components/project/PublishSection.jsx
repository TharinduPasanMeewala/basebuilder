...
export default function PublishSection({ project, onRefresh }) {
  ...
  const runPublish = async () => {
    ...
    try {
      ...
      const [reqs, entities, pages, workflows, apis, blueprints, devPlans, designs] = await Promise.all([
        ...
        base44.entities.ProjectDesign.filter({ project_id: project.id }, '-updated_date', 1),
      ]);
...
Return ONLY a JSON object (no markdown fences):
{
  ...
  "layout_component": "complete src/components/Layout.jsx with sidebar/nav"
      ...
      // Build generated code bundle from the same source of truth used by preview/design.
      const generatedFiles = generateCodeFiles(project, { entities, pages, requirements: reqs, design });
      const configFiles = {};
      const sourceFiles = {};
      generatedFiles.forEach(file => {
        if (file.path.startsWith('src/')) sourceFiles[file.path.replace(/^src\//, '')] = file.content;
        else configFiles[file.path] = file.content;
      });
      configFiles['.env.example'] = envVars.join('\n');

      const code = {
        '⚙️ config': configFiles,
        '🎨 src': sourceFiles,
        '🗄️ database': {

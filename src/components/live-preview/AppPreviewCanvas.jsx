...
function PageBody({ page, entities }) {
  ...
  if (page.type === 'dashboard') return <div className="grid grid-cols-2 gap-3">{entities.slice(0,4).map((e,i)=><div key={e.id} className="ai-preview-card rounded-xl border border-border p-4"><p className="text-xs text-muted-foreground">{e.display_name || e.name}</p><p className="text-2xl font-bold text-foreground mt-1">{(i+1)*12}</p></div>)}</div>;
...
export default function AppPreviewCanvas({ pages, entities }) {
  ...
  return <div className="ai-preview-shell rounded-xl border border-border overflow-hidden p-3 min-h-[540px]"><div className="ai-preview-browser rounded-xl border border-border overflow-hidden min-h-[520px]"><div className="h-9 bg-muted/40 border-b border-border flex items-center gap-2 px-3"><span className="w-3 h-3 rounded-full bg-destructive/70"/><span className="w-3 h-3 rounded-full bg-yellow-500/70"/><span className="w-3 h-3 rounded-full bg-accent/70"/><span className="ml-3 text-xs text-muted-foreground">AI generated app preview · CSS applied</span></div><div className="flex min-h-[480px]"><aside className="ai-preview-sidebar w-52 text-sidebar-foreground p-4"><h3 className="font-bold text-white text-sm mb-5">{active?.module || 'App'}</h3><nav className="space-y-1">{pages.map(p=>{const Icon=pageIcon[p.type]||LayoutDashboard;return <button key={p.id} onClick={()=>setActiveId(p.id)} className={`w-full flex items-center gap-2 rounded-md px-3 py-2 text-xs text-left transition-all ${active?.id===p.id?'bg-sidebar-active text-white ai-preview-primary-glow':'hover:bg-sidebar-hover'}`}><Icon className="w-3.5 h-3.5" />{p.name}</button>})}</nav></aside><main className="flex-1 p-6 bg-background"><div className="mb-6"><p className="text-xs text-primary font-medium capitalize">{active?.type || 'page'} · {active?.route}</p><h2 className="text-2xl font-bold text-foreground mt-1">{active?.name}</h2><p className="text-sm text-muted-foreground mt-1">{active?.description}</p></div><PageBody page={active || {}} entities={entities} /></main></div></div></div>;
}